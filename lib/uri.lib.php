...
function get_pretty_url($folder, $no = '', $query_string = '', $action = '')
{
    ...
    if ($config['cf_bbs_rewrite']) {
        ...
        if ($folder === 'content' && $no) {     // 내용관리
            ...
            if ($no) {
                ...
                if ($config['cf_bbs_rewrite'] > 1) {
                    ...
                    $get_write = get_write($koi['write_prefix'] . $folder, $no, true);
        ...
        if (in_array($folder, $boards)) {
            $url = KOI_BBS_URL . '/board.php?bo_table=' . $folder;
            if ($no) {
                $url .= '&amp;wr_id=' . $no;
            }
            if ($query_string) {
...
function short_url_clean($string_url, $add_qry = '')
{
    ...
    if (isset($config['cf_bbs_rewrite']) && $config['cf_bbs_rewrite']) {
        ...
        if (stripos(preg_replace('/^https?:/i', '', $string_url), preg_replace('/^https?:/i', '', KOI_BBS_URL)) === false || !in_array($page_name, $array_page_names)) {   //게시판이 아니면 리턴
        ...
        */
        ...
        if (isset($url['host'])) {
            ...
            $array_file_paths = run_replace('url_clean_page_paths', array('/' . KOI_BBS_DIR . '/board.php', '/' . KOI_BBS_DIR . '/write.php', '/' . KOI_BBS_DIR . '/content.php'));

            $str_path = isset($url['path']) ? $url['path'] : '';
            $http = (isset($isSecure) && $isSecure) ? 'https://' : 'http://';
...
function generate_seo_title($string, $wordLimit = KOI_SEO_TITEL_WORD_CUT)
{
    $separator = '-';
...
function get_nginx_conf_rules($return_string = false)
{
    ...
    $get_path_url = parse_url(KOI_URL);

    $base_path = isset($get_path_url['path']) ? $get_path_url['path'] . '/' : '/';

    $rules = array();

    $rules[] = '#### ' . KOI_VERSION . ' nginx rules BEGIN #####';
    ...
    $rules[] = "rewrite ^{$base_path}content/([0-9a-zA-Z_]+)$ {$base_path}" . KOI_BBS_DIR . "/content.php?co_id=$1&rewrite=1 break;";
    $rules[] = "rewrite ^{$base_path}content/([^/]+)/$ {$base_path}" . KOI_BBS_DIR . "/content.php?co_seo_title=$1&rewrite=1 break;";
    $rules[] = "rewrite ^{$base_path}rss/([0-9a-zA-Z_]+)$ {$base_path}" . KOI_BBS_DIR . "/rss.php?bo_table=$1 break;";
    $rules[] = "rewrite ^{$base_path}([0-9a-zA-Z_]+)$ {$base_path}" . KOI_BBS_DIR . "/board.php?bo_table=$1&rewrite=1 break;";
    $rules[] = "rewrite ^{$base_path}([0-9a-zA-Z_]+)/write$ {$base_path}" . KOI_BBS_DIR . "/write.php?bo_table=$1&rewrite=1 break;";
    $rules[] = "rewrite ^{$base_path}([0-9a-zA-Z_]+)/([^/]+)/$ {$base_path}" . KOI_BBS_DIR . "/board.php?bo_table=$1&wr_seo_title=$2&rewrite=1 break;";
    $rules[] = "rewrite ^{$base_path}([0-9a-zA-Z_]+)/([0-9]+)$ {$base_path}" . KOI_BBS_DIR . "/board.php?bo_table=$1&wr_id=$2&rewrite=1 break;";
    $rules[] = '}';
    $rules[] = '#### ' . KOI_VERSION . ' nginx rules END #####';
...
function get_mod_rewrite_rules($return_string = false)
{
    ...
    $rules[] = '#### ' . KOI_VERSION . ' rewrite BEGIN #####';
    ...
    $rules[] = 'RewriteRule ^content/([0-9a-zA-Z_]+)$  ' . KOI_BBS_DIR . '/content.php?co_id=$1&rewrite=1  [QSA,L]';
    $rules[] = 'RewriteRule ^content/([^/]+)/$  ' . KOI_BBS_DIR . '/content.php?co_seo_title=$1&rewrite=1      [QSA,L]';
    $rules[] = 'RewriteRule ^rss/([0-9a-zA-Z_]+)$  ' . KOi_BBS_DIR . '/rss.php?bo_table=$1        [QSA,L]';
    $rules[] = 'RewriteRule ^([0-9a-zA-Z_]+)$  ' . KOI_BBS_DIR . '/board.php?bo_table=$1&rewrite=1      [QSA,L]';
    $rules[] = 'RewriteRule ^([0-9a-zA-Z_]+)/([^/]+)/$ ' . KOI_BBS_DIR . '/board.php?bo_table=$1&wr_seo_title=$2&rewrite=1      [QSA,L]';
    $rules[] = 'RewriteRule ^([0-9a-zA-Z_]+)/write$  ' . KOI_BBS_DIR . '/write.php?bo_table=$1&rewrite=1    [QSA,L]';
    $rules[] = 'RewriteRule ^([0-9a-zA-Z_]+)/([0-9]+)$  ' . KOI_BBS_DIR . '/board.php?bo_table=$1&wr_id=$2&rewrite=1  [QSA,L]';
    $rules[] = '</IfModule>';
    $rules[] = '#### ' . KOI_VERSION . ' rewrite END #####';
...
function update_rewrite_rules()
{
    ...
    if ($is_apache) {
        ...
        if ((!file_exists($save_path) && is_writable(KOI_PATH)) || is_writable($save_path)) {
