...
function get_list_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    if($tname) {
        if($edt) {
            // 오리지날 이미지
            $ori = KOI_URL.$data_path;
            // 썸네일 이미지
            $src = KOI_URL.str_replace($filename, $tname, $data_path);
        } else {
            $ori = KOI_DATA_URL.'/file/'.$bo_table.'/'.$filename;
            $src = KOI_DATA_URL.'/file/'.$bo_table.'/'.$tname;
        }
    } else {
        return false;
    }
...
function get_view_thumbnail($contents, $thumb_width=0)
{
    ...
    for($i=0; $i<count($matches[1]); $i++) {
        ...
        // 이미지 path 구함
        $p = parse_url($src);
        if(strpos($p['path'], '/'.KOI_DATA_DIR.'/') != 0)
            $data_path = preg_replace('/^\/.*\/'.KOI_DATA_DIR.'/', '/'.KOI_DATA_DIR, $p['path']);
        else
            $data_path = $p['path'];

        $srcfile = KOI_PATH.$data_path;
        ...
        if(is_file($srcfile)) {
            ...
            if ($width) {
                $thumb_tag = '<img src="'.KOI_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'" width="'.$width.'" height="'.$height.'"/>';
            } else {
                $thumb_tag = '<img src="'.KOI_URL.str_replace($filename, $thumb_file, $data_path).'" alt="'.$alt.'"/>';
            }

            // $img_tag에 editor 경로가 있으면 원본보기 링크 추가
            $img_tag = $matches[0][$i];
            if(strpos($img_tag, KOI_DATA_DIR.'/'.KOI_EDITOR_DIR) && preg_match("/\.({$config['cf_image_extension']})$/i", $filename)) {
                $imgurl = str_replace(KOI_URL, "", $src);
                $thumb_tag = '<a href="'.KOI_BBS_URL.'/view_image.php?fn='.urlencode($imgurl).'" target="_blank" class="view_image">'.$thumb_tag.'</a>';
            }
...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    ...
    if (!is_dir($target_path)) {
        @mkdir($target_path, KOI_DIR_PERMISSION);
        @chmod($target_path, KOI_DIR_PERMISSION);
    }
    ...
    if($is_large) {
        ...
        if($is_crop) {
            ...
            if ( !((defined('KOI_USE_THUMB_RATIO') && false === KOI_USE_THUMB_RATIO) || (defined('KOI_THEME_USE_THUMB_RATIO') && false === KOI_THEME_USE_THUMB_RATIO)) ){
                if($src_w > $src_h) {
    ...
    if($size[2] == 1) {
        ...
        if(!defined('KOI_THUMB_PNG_COMPRESS'))
            $png_compress = 5;
        else
            $png_compress = KOI_THUMB_PNG_COMPRESS;

        imagepng($dst, $thumb_file, $png_compress);
    } else {
        if(!defined('KOI_THUMB_JPG_QUALITY'))
            $jpg_quality = 90;
        else
            $jpg_quality = KOI_THUMB_JPG_QUALITY;

        imagejpeg($dst, $thumb_file, $jpg_quality);
    }

    @chmod($thumb_file, KOI_FILE_PERMISSION); // 추후 삭제를 위하여 파일모드 변경
